...
入力定義では、リクエストのキーごとにテキストエリア、セレクトボックス、ファイルなどのコンポーネントを指定できる。

![源内Webで利用できる入力定義コンポーネント](images/源内入力定義コンポーネント.png)
...
チーム管理画面で、AWS Specialistを公開する対象チームを選択する。

![AIアプリを登録する対象チームの選択](images/チーム管理画面.png)
